...
router.post('/signup', async (req, res) => {
    try {
        ...
        if (req.app.get('isUsingMockDB')) {
            ...
            const token = jwt.sign({ email: newUser.email, userId: newUser._id }, process.env.JWT_SECRET || 'secret_key_ivo', { expiresIn: '1h' });
...
router.post('/login', async (req, res) => {
    try {
        ...
        if (req.app.get('isUsingMockDB')) {
            ...
            const token = jwt.sign({ email: user.email, userId: user._id }, process.env.JWT_SECRET || 'secret_key_ivo', { expiresIn: '1h' });
